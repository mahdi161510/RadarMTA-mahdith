...
function makeRoute(destinationX, destinationY, uTurned)
	...
	if disallowedZones[zoneName] or disallowedZones[cityName] then
		addNotification("Masiri Bare Maghsad Shoma Iaft Nashod.", "error2")
	...
	if not uTurned then
		selectedRouteSound = playSound("radar/gps/sounds/uticel.mp3")
	end
end
...
function playGPSSound(sounds)
	...
	currentGPSSound = playSound("radar/gps/sounds/ding.mp3")
...
function playNextGPSSound(sounds, count)
	currentGPSSound = playSound("radar/gps/sounds/" .. sounds[count] .. ".mp3")
...
function playOneGPSSound(sound)
	...
	currentGPSSound = playSound("radar/gps/sounds/" .. sound .. ".mp3")
end
